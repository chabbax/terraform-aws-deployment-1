...
variable "recommendation_exec_bin" {
  type        = string
  description = "The executable binary"

  validation {
    condition     = length(var.recommendation_exec_bin) > 0
    error_message = "The recommendation_exec_bin must not be empty."
  }
}

variable "recommendation_exec_params" {
  type        = string
  description = "The executable parameters"

  validation {
    condition     = length(var.recommendation_exec_params) > 0
    error_message = "The recommendation_exec_params must not be empty."
  }
}

variable "recommendation_matrix_path" {
  type        = string
  description = "The reccomedation matrix parameter"

  validation {
    condition     = length(var.recommendation_matrix_path) > 0
    error_message = "The recommendation_matrix path must not be empty."
  }
}
